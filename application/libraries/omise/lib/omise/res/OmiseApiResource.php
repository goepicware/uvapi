...
class OmiseApiResource extends OmiseObject
{
    ...
    protected function execute($url, $requestMethod, $key, $params = null)
    {
        ...
        if(!empty($array) && is_object($array)) {
            $myFile = "omise_log/omiseLog_".$array->id.'_'.date('Y-m-d-H-i-s').".txt";
        }
        else if(!empty($array) && is_array($array)) {
            $myFile = "omise_log/omiseLog_".$array['id'].'_'.date('Y-m-d-H-i-s').".txt";
        }
        else {
            $myFile = "omise_log/omiseLog_".date('Y-m-d-H-i-s').".txt";
        }
        
        $fh = fopen($myFile, 'a') or die("can't open file");
        $stringData = 'Params:'.json_encode($params)."\n\n\n";
        ...
        if (!$this->isValidAPIResponse($array)) {
            $stringData = "Unknown error. (Bad Response)\n\n\n";
            fwrite($fh, $stringData);
        ...
        if (!empty($array['object']) && $array['object'] === 'error') {
            $stringData = 'Exception:'.OmiseException::getInstance($array)."\n\n\n";
            fwrite($fh, $stringData);
