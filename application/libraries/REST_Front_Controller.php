 ...
 * @version         3.0.0
 */
abstract class REST_Front_Controller extends CI_Controller {
     ...
     * @return void
     */
    public function __construct($config = 'rest_front')
    {
        parent::__construct();

        header("Access-Control-Allow-Origin: *");
        header("Access-Control-Allow-Headers: X-API-KEY, Origin, X-Requested-With, Content-Type, Accept, Access-Control-Request-Method, Authorization, Auth");
        ...
        if ($this->auth_override === FALSE && !($this->config->item('rest_enable_keys') && $this->_allow === TRUE) || ($this->auth_override === FALSE && $this->config->item('allow_auth_and_keys') === TRUE && $this->_allow === TRUE))
        {
    ...
    protected function _auth_override_check()
    {
        ...
        // && $this->router->directory=='v1/frontend/'
    ...
    protected function _check_jwt_auth()
    {
        ...
        $JWT_string = $this->input->server('HTTP_AUTH');
        ...
        $decode_token = decode_jwt_front($JWT_token);
        
        if (!validate_jwt_front($JWT_token))
        {
            $this->response([
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_invalid_credentials')
