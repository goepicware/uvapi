...
class Faqs extends REST_Controller
{
		...
		if (isset($headers['Authorization'])) {
			...
			if ($decodedToken['status']) {
				...
				if (!empty($company_id)) {
					...
					$result = $this->Mydb->get_record($this->primary_key . ', faq_title', $this->table, $where);
					if (!empty($result)) {
						$this->Mydb->delete($this->table, array($this->primary_key => $result[$this->primary_key]));

						createAuditLog("FAQ", stripslashes($result['faq_title']), "Delete", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
	...
	public function addedit($edit_id = null)
	{
		...
		if ($action == 'add') {
			...
			createAuditLog("FAQ", stripslashes(post_value('faq_title')), "Add", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		} else {
			$data = array_merge(
				$data,
				array(
			...
			createAuditLog("FAQ", stripslashes(post_value('faq_title')), "Update", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		}
	}
