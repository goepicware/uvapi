...
Description		: Staic Blocks

 ***************************/
defined('BASEPATH') or exit('No direct script access allowed');
require APPPATH . '/libraries/REST_Controller.php';
class Staticblocks extends REST_Controller
{
	function __construct()
	{
		...
		$this->table = "staticblocks";
		$this->load->library('common');
		$this->label = "Static Blocks";
		$this->load->library('Authorization_Token');
		$this->primary_key = 'staticblocks_id';
		$this->company_id = 'staticblocks_company_id';
	}
	...
	public function list_get()
	{
		...
		if (isset($headers['Authorization'])) {
			...
			if ($decodedToken['status']) {
				$select_array = array('staticblocks_id ', 'staticblocks_title', 'staticblocks_status', 'staticblocks_slug', 'staticblocks_login_status');
				$limit = $offset = $like = '';
				...
				if (!empty($status)) {
					$where = array_merge($where, array('staticblocks_status' => $status));
				}
				if (!empty($name)) {
					$like = array("staticblocks_title" => $name);
				}
	...
	public function add_post()
	{
		...
		if (isset($headers['Authorization'])) {
			$decodedToken = $this->authorization_token->validateToken($headers['Authorization']);
			if ($decodedToken['status']) {
				$this->form_validation->set_rules('static_title', 'Title', 'required|trim|strip_tags|callback_title_exists');
		...
		if (isset($headers['Authorization'])) {
			...
			if ($decodedToken['status']) {
				...
				if (!empty($company_id)) {
					...
					$result = $this->Mydb->get_record($this->primary_key . ', staticblocks_title', $this->table, $where);
					if (!empty($result)) {
						$this->Mydb->delete($this->table, array($this->primary_key => $result[$this->primary_key]));

						createAuditLog("Static Block", stripslashes($result['staticblocks_title']), "Delete", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
	...
	public function addedit($edit_id = null)
	{
		...
		$data = array(
			'staticblocks_title' => post_value('static_title'),
			'staticblocks_description' => $this->input->post('static_description', FALSE),
			'staticblocks_images_desc' => post_value('static_gallery_img_desc'),
			'staticblocks_login_status' => post_value('staticblocks_login_status'),
			'staticblocks_status' => post_value('status'),
		);
		$staticblocks_images = post_value('staticblocks_images');
		if (!empty($staticblocks_images)) {
			$data = array_merge($data, array('staticblocks_images' => $staticblocks_images));
		}
		$company_id = decode_value($this->input->post('company_id'));
		$company_admin_id = decode_value($this->input->post('company_admin_id'));
		$getCompanyDetails = getCompanyUniqueID($company_id);
		if ($action == 'add') {
			...
			$slug = make_slug($title, $this->table, 'staticblocks_slug', array($this->company_id => $company_id));

			$data = array_merge(
				$data,
				array(
					'staticblocks_slug'		=> $slug,
					'staticblocks_company_id' => $company_id,
					'staticblocks_unquie_id' 	=> (!empty($getCompanyDetails)) ? $getCompanyDetails : '',
					'staticblocks_created_on' => current_date(),
					'staticblocks_created_by' => $company_admin_id,
					'staticblocks_created_ip' => get_ip()
				)
			);

			$this->Mydb->insert($this->table, $data);
			createAuditLog("Static Block", stripslashes(post_value('static_title')), "Add", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		} else {
			$slug = make_slug($title, $this->table, 'staticblocks_slug', array("$this->primary_key !=" => $edit_id, $this->company_id => $company_id));
			$data = array_merge(
				$data,
				array(
					'staticblocks_slug'		=> $slug,
					'staticblocks_updated_on' => current_date(),
					'staticblocks_updated_by' => $company_admin_id,
					'staticblocks_updated_ip' => get_ip()
				)
			);
			$this->Mydb->update($this->table, array($this->primary_key => $edit_id), $data);
			createAuditLog("Static Block", stripslashes(post_value('static_title')), "Update", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		}
	}
	public function title_exists()
	{
		...
		$company_id = decode_value($this->input->post('company_id'));
		$where = array(
			'staticblocks_title' => trim($static_title),
