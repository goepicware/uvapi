...
class Emailtemplate extends REST_Controller
{
		...
		if (isset($headers['Authorization'])) {
			...
			if ($decodedToken['status']) {
				...
				if (!empty($company_id)) {
					...
					$result = $this->Mydb->get_record($this->primary_key . ', email_subject', $this->table, $where);
					if (!empty($result)) {
						$this->Mydb->delete($this->table, array($this->primary_key => $result[$this->primary_key]));

						createAuditLog("Email Template", stripslashes($result['email_subject']), "Delete", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
	...
	public function addedit($edit_id = null)
	{
		...
		if ($action == 'add') {
			...
			createAuditLog("Email Template", stripslashes(post_value('email_title')), "Add", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		} else {
			$data = array_merge(
				$data,
				array(
			...
			createAuditLog("Email Template", stripslashes(post_value('email_title')), "Update", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		}
	}
