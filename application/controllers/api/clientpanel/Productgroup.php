...
class Productgroup extends REST_Controller
{
		...
		if (isset($headers['Authorization'])) {
			...
			if ($decodedToken['status']) {
				...
				if (!empty($company_id)) {
					...
					$result = $this->Mydb->get_record($this->primary_key . ', pro_group_name', $this->table, $where);
					if (!empty($result)) {
						...
						createAuditLog("Group", stripslashes($result['pro_group_name']), "Delete", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
	...
	public function addedit($edit_id = null)
	{
		...
		if ($action == 'add') {
			...
			createAuditLog("Group", stripslashes(post_value('group_name')), "Add", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		} else {
			$data = array_merge(
				$data,
				array(
			...
			createAuditLog("Group", stripslashes(post_value('group_name')), "Update", $company_admin_id, 'Web', '', $company_id, $getCompanyDetails);
		}
		if (!empty($edit_id)) {
