...
if (!function_exists('catering_status')) {
	function catering_status()
	{
		...
		$status = "";
		$where_in = array(
			'company_id' => get_company_id(), 'company_app_id' => get_company_app_id(),
...
if (!function_exists('createAuditLog')) {
	function createAuditLog($module_name, $description, $action_type, $user_id, $via, $log_json, $company_id, $uniqueId)
	{
		$CI = &get_instance();
		$insert_array = array(
			...
			"audit_company_unique_id" => $uniqueId

		);
		$CI->Mydb->insert("audit_reports", $insert_array);
	}
}
